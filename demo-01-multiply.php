...
<html>
	<head>
		<title>01 Multiply - Interactive (Advanced) Demo</title>
		<meta charset="utf-8" />
			...
			<section id="one" class="main style1">
				<div class="container">
					<div class="row 150%">
						<div class="12u">
							<header class="major">
								<h2>Part I - Multiply Bug Fix</h2>
							</header>
              <p>
                Hello there, PHP developer!  Since you are planning to use the PHP Test Fixture (as authored by <a href="https://github.com/DonaldKellett" target="_blank">DonaldKellett</a>) for your own purposes (or for the purposes of your company), you will need to know basic PHP syntax in order to operate it.  So, let's start with something simple.  Provided for you is some PHP code meant to define a function <code>multiply</code> which accepts two numbers, <strong>a</strong> and <strong>b</strong> and returns the product of the two numbers.  However, the code is not running properly for some reason.  Can you figure out where the problem is?
              </p>
              <h3>Credits</h3>
              <p>
                This section of the Demo is inspired by the introductory <strong>Multiply (8kyu)</strong> exercise in <a href="https://www.codewars.com" target="_blank">Codewars</a>.
              </p>
						</div>
                  ...
                  <div class="12u">
                    <p>
                      <textarea name="code_editor" style="resize: none; height: 500px; background-color: black; color: white; font-family: monospace"><?php
											define("DEFAULT_SETUP", "multiply(a, b) {
  a * b
}");
            ...
            <div class="6u 12u(medium)">
							...
							if ($_SERVER['REQUEST_METHOD'] === 'POST') {
								...
								$test = new Test;
								$test->describe("<span style='color:white'>The 'multiply' function</span>", function () {
									$GLOBALS['test']->it("<span style='color:white'>(Your submitted code) should not throw an error</span>", function () {
									...
									if ($GLOBALS['first_test_passed']) {
										$GLOBALS['test']->it('<span style="color:white">should be defined</span>', function () {
											$GLOBALS['second_test_passed'] = $GLOBALS['test']->expect(function_exists('multiply'), "'multiply' should be defined");
										});
									}
									if ($GLOBALS['second_test_passed']) {
										$GLOBALS['test']->it('<span style="color:white">should work for some fixed tests</span>', function () {
											...
											$GLOBALS['test']->assert_equals(multiply(2, 3), 6);
											$GLOBALS['test']->assert_equals(multiply(3, 2), 6);
											$GLOBALS['test']->assert_equals(multiply(3, 5), 15);
											$GLOBALS['test']->assert_equals(multiply(5, 3), 15);
											$GLOBALS['test']->assert_equals(multiply(2, 4), 8);
											$GLOBALS['test']->assert_equals(multiply(4, 2), 8);
											$GLOBALS['test']->assert_equals(multiply(9, 9), 81);
											$GLOBALS['test']->assert_equals(multiply(15, 12), 180);
										});
										$GLOBALS['test']->it('<span style="color:white">should work for random tests</span>', function () {
											for ($i = 0; $i < 100; $i++) {
												...
												$actual = multiply($a, $b);
												$GLOBALS['test']->assert_equals($actual, $expected, "Your 'multiply' function did not return the correct result");
											}
										});
									}
								});
...
?>");
									...
									echo "<p>
										<a class='button' href='demo-02-broken-greetings.php'><span class='fa fa-arrow-right'></span> Next Challenge</a>
